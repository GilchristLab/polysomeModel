...
\textcurrency Current Address: Department of Biology, University of Pennsylvania, Philadelphia, PA, USA % change symbol to "\textcurrency a" if more than one current address note
...
The decapping rate between the capped and uncapped system was estimated from the protein half-lives from Presnyak (2015) for yeast (Figure 2B) and Sorenson (2018) for Arabidopsis (Figure 2D)~\cite{RN27,RN28}.
...
Finally, the fraction of mRNA predicted in the capped and decapped class  are consistent with population wide estimates (\cite{RN4}, Figure 10). 
