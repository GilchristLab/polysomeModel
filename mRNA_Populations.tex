...
\item Focusing now steady state abundance of the ribosome occupied decapped mRNA polysome classes, i.e.  $\mhat_i^*$ where $i > 0$, we find that the distribtuion of $\mhat^*_i$  depends on the gene specific ribosome elongation rate $\tau_0$ (where `elongation`  includes the ribosome's reading of the mRNA's stop codon) and the distribution of capped mRNA $\mhat_i$ with $i > 0$ (Figure 4).
  This finding implies that because the density of $\mhat^*_i$ monotonically decreases with $i$, the distribution of decapped mRNA polysome classes is skewed and dominated by lower polysomal classes.
...
\item Increasing marking $\mu$ rate and scaled translation initiation rate $\kappa/\tau$ both increase the proportion of transcripts in the decapped state.
  ...
  \end{enumerate}
\end{enumerate}
\mmpar{The following points be incorporated into the previous discussion of MRL and protien production.} 
