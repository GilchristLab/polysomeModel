...
The number of ribosomes bound at a given time is referred to as ribosome load, and using this value a population of mRNA are classiﬁed.
...
\multicolumn{Model Parameters} & \\ \hline
...
where $\tau_0$ is potentially gene specific parameter that describes the average elongation rate of a ribosome along the protein coding sequence of that gene.

 
In contrast, protein translation occurs independelty of whether or not an mRNA is unmarked or marked for deletion.
...
The ($\imax-1^{th}$) row has been suppressed in Equation~\ref{eq:unmarked_matrix}, but it can be generated using the formula included for the $i^{th}$ row.
