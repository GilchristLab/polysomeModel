...
\subsection{Marking rate and ribosomal load determine mRNA distribution between capped and decapped states}
\begin{enumerate}
...
\begin{enumerate}
  ...
  \item We produced output across all scaled initiation values $\kappa'$ and under the 1\textsuperscript{st}, 50\textsuperscript{th} and 99\textsuperscript{th} percentiles for marking rates in both yeast and Arabidopsis (Figure 8). 
...
\caption{The mean ribosomal density on a transcript is dependent on coding sequence length. MRL per transcript is higher for longer transcripts. A) Capped state B) Decapped state C) Full system. Yeast parameters were used \imax =  4 (1\textsuperscript{st} percentile),  39 (50\textsuperscript{th} percentile), 194 (99\textsuperscript{th} percentile), low marking rate ($2.2\times 10^{-4}$ 1/s), over the full scaled initiation range 0.0001- 0.5.}
%\centering Red: decapped class, Green: capped class, Blue: Total= capped+decapped classes}
\end{figure}
\clearpage
...
\begin{enumerate}
\item Protein production rate (PPR) is a function of full system MRL $\times \tau$ and plots normalized to highest protein output are shown in Figure 10.
\begin{enumerate}
  \item As the marking rate $\mu$ increases it reduces the capped and uncapped MRL as well as shifting transcript abundance to the decapped state (Figure 10A).
  ...
  \item A surprising finding from out model is that when $\mu$ is high ($5.7\times 10^{-3}$ 1/s), 41\% of all protein production can arise from the decapped state (Figure 10 D).
...
\caption{Estimated average protein production in yeast.  A) Protein production across different marking rates  low $\mu$ ($2.2\times 10^{-4}$ 1/s), median $\mu$ ($1.7\times 10^{-3}$ 1/s), high $\mu$ ($5.7\times 10^{-3}$ 1/s). Total protein production is normalized to the maximal protein production across all parameters. B-D) Contribution of capped and decapped states to total protein production. B) Low marking C) Medium marking D) High marking. }
%\centering Red: decapped class, Green: capped class, Blue: Total= capped+decapped classes}
\end{figure}
\clearpage
...
\begin{enumerate}
  ...
  \item This result is impressive as the model performs well despite no model fitting being performed and supports the idea that the model is a useful representaiton of the complex processes underlying protein production.
