...
\end{equation} 
where \kappao is the system-wide parameter that controls initiation.
...
\end{eqnarray*}
}\mymarginpar{These equations are redundant.}
...
\mymarginpar{mike: I've never seen equations presented in 'figures' this way. 
  Please simply make them equations and move any information in the caption that is not in the text to the text.
}
...
\mymarginpar{mike: ditto previous note.}
...
\begin{figure}[ht]
\caption{The matrix $\bs{U}$ is $(\imax+1\times \imax+1)$ dimensional and is tri-diagonal with non-zero entries on the diagonal, super-, and sub-diagonals.
In the representation below, all blank entries are 0.}  %All \footnotesize\emph{Note: The ($\imax-1^{st}$) row has been suppressed in this representation but can be seen in Figure~\ref{fig:unmarked_matrix}}}
...
\noindent BEGIN CRITICAL NOTE BY MIKE

If equation (\ref{eq:anyl_unmarked_imax}) is correct, then it seems we've missed a very simplification that would simplify our calculations \emph{immensely}.
...
\intertext{Based on my understanding of mathematics,}
  ...
  &= \frac{ \prod_i a_i \prod_i b_i }{\prod_i a_i \prod_i \left(c_i + b_i\right)\right)}\\
  &= \frac{\prod_i b_i }{\prod_i \left(c_i + b_i\right)\right)}\\
\end{align}
If this is correct and we define $a_i = 1/\GCD[\imax, i]$ and $b_i \kappa_0$ and $c_i (\mu_0 + i \tau_0)$, then equation (\ref{eq:anyl_unmarked_imax}) can be rewritten as,
\begin{equation}
  \hat{m}_{\imax}= \frac{ prod_{i=1}^{\imax} \kappa_0 i}{prod_{i=1}^{\imax} \left(\mu_0 + i \left(\tau_0 + \kappa_0\right)\right)} 
\end{equation}
If this is the case, then I'm both elated and embarrassed as hell we didn't see it earlier.

\noindent END CRITICAL NOTE
...
%\begin{equation*}
%Det[\bs{U}]=(-1)^{\imax+1}\mu_0\prod_{i=1}^{\imax}(\frac{i}{\GCD[\imax,i]}\kappa_0+\frac{\imax}{\GCD[\imax,i]}(\mu_0+i\tau_0))\frac{1}{\frac{\imax}{\GCD[\imax,i]}}
%\end{equation*}
%Which simplifies to:
\begin{equation}\label{eq:U_det}
Det[\bs{U}]=(-1)^{\imax+1}\mu_0\prod_{i=1}^{\imax}{\left(\frac{i}{\imax}(\kappa_0)+(\mu_0+i\tau_0)\right)}.
\end{equation}
