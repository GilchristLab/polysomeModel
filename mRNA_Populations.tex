...
Another way of comparing relative protein output (RPO) is to calculate the mean ribosomal load (MRL) for each state using \label{eq:Expected_ribo_load} and multiply is by the total number of transcripts in their respective state. The full systems protein production can be found with \label{eq:System_ribo_load}. In our model the MRL of the capped system will always be equal to or greater than the decapped MRL (Figure 10 A). As the scaled initiation rate increases and the MRL of the capped class increases, the distributional properties of the uncapped class mean that it will always be proportionally less to the MRL of the capped class.  

The RPO of a state is dependent on both the MRL of each state and the total transcript population of each state (Figure 10B). As the scaled initiation rate increases RPO will increase. The marking rate $\mu$ plays an important role in controlling both the underlying mRNA population and the distribution of transcripts in the capped state. Therefore, and increase in marking rate reduces RPO through two mechanisms. Each of the three cases in Figure 10 B, has been broken down into the RPO contributions from the capped and decapped states (Figure 10 C-E). A surprising finding from out model is that under certain biologically relavant parameter combinations, almost half of all protein production can arise from the decapped state. This can be seen inf Figure 10 F, wherea substantial portion of all protein arises from the decapped state. The reason behind this is despite the the relative MRL of the decapped state being lower than the capped state as scaled initiation rises, the amount of mRNA in the decapped state rises faster.
...
\subsection{General Model Findings}

\item Our model combines the processes of transcription, translation and mRNA degradation (through 5' decapping) in order to predict mRNA populations and the protein production that arises from them. 
	\begin{enumerate}
			\item Model predictions of the mRNA distribution across polysome classes match what would be expected in polysome gradients (Lokdarshi 2020, Dasgupta 2023), Ribo-seq (Weinberg 2016), and single molecule measurements of translation (Morisaki 2016, Yan 2016, Wang 2016, Wu 2016).   
		\item Analytical and numerical solutions show that the transcription rate $\lambda$ is only a scaling factor across the whole system. 
		\item  The population size of capped mRNAs $m_{tot}$ is solely determined by $\lambda$ divided by the marking rate $\mu$.
		\item The $m_0^*$ class is also independent of translation and is determined by $\lambda$ divided by the clearance rate $\delta$.
		\item While the remaining classes in the decapped state depend on the elongation/termination rate $\tau_0$ and the underlying distribution of the mRNA in the capped state. Our model predictions of the capped to decapped mRNA ratios also align with empirical measurements (Pelechano 2015)
		\item A unique feature of our model is that we can separate the translational behavior between the capped and decapped classes.
		\item The distribution of mRNAs across the whole system can be unimodal or bimodal depending on how transcripts are distributed between the capped and decapped states.
		\item The capped mRNA distribution appears to be a truncated gaussian, with bounds at $i$ = 0 and \imax.		
		\item  The decapped mRNA distribution is skewed towards lower polysomal classes.
		\item The MRL of the capped class will always be greater or equal to the MRL of the Decapped class.
		\item Finally, under steady state conditions the MRL is independent of the protein length, this too is corroborated by single molecule imaging of translation (Wu 2016).
	\end{enumerate}

\subsection{The interplay between marking rate and scaled initiation rates affects MRL, proportion of mRNA in the capped state and protein production}
...
\item Increasing marking rate and scaled initiation rate both increase the proportion of transcripts in the decapped state.
	\begin{enumerate}
	\item The model formalizes the interplay between mRNA decapping and translation initiation. 
	\item As expected, increasing marking rates result in an increase in the number of transcripts in the decapped state.
	\item However, increasing the scaled initiation rate also leads to an increase of transcripts in the decapped state. 
	\item As MRL in the capped classes increases, transcripts enter the decapped state at higher polysomal classes and thus take longer to reach $m_0^*$.
	\item Model predictions of the fraction of mRNA in the decapped class fall with the 12.4\% population wide empirical estimate from Pelechano 2015. 
	\end{enumerate}

\item Increasing marking rate also decreases the MRL proportional to increasing scaled initiation rates. 
	\begin{enumerate}	
		\item  Under low marking rates ($\mu ~ 10^{-6}$ 1/s, Half-life ~ 1,500 minutes), MRL for the system comes almost entirely from the capped state. 
		\item As marking rate increases, a bimodal distribution starts to appear.
		\item This bimodal distribution consists of  a mixture of the MRL from the capped and decapped states. 
		\item As marking rates increase further, a greater proportion of the MRL arises from the decapped states.
		\item As the decapped MRL $\leq$ the capped MRL, the whole system MRL decreases.   
	\end{enumerate} 
...
\item A surprising prediction from our model is that genes with  high marking rates ($\mu ~ 5 \times 10^{-3}$) can have almost half of their protein produced in the decapped state. 
	\begin{enumerate}
	\item As marking rates increase, the difference between capped and decapped MRL decrease as more transcripts populate the decapped state. 
	\item The ability to predict decapped mRNA population size and translational activity is a unique feature of our model.
...
\item	The 5' mRNA decay pathway presented in our model most closely resembles that of co-translational decay, and in Arabidopisis 68\% of mRNAs are degraded this way (Sorenson 2018).
	\begin{enumerate}
		...
		\item The interplay of these mechanisms is of interest as mRNA stability is essential for protein expression for biotech and mRNA vaccines. (Cheng 2023 viruses, Boo and Kim 2020+) 
	\end{enumerate}
...
\item Currently there is a debate whether mRNA stability is regulated primarily through the protective effects of ribosomal association or through the suboptimal codons causing ribosomal stalling and the subsequect ribosome associated decay pathways  (Chan 2018 elife).
	...
	\item Our model does not consider codon specific effects such as pausing sites, difficult to fold regions of a protein or codon optimality (Wu and Bazzini 2023).
	\item Splitting each polysome class into two regions would approximate a ribosome flow model of only two regions, bounded by the pausing site.
