...
Thus the initiation function for the decapped set of mRNAs is $\kappa(i) = 0$.
In contrast,  ribosomes can initiate translation on a capped mRNA.
...
Finally, we assume that capped mRNAs are decapped for degradation rate independent of their ribosome load, i.e. $\mu(i)=\mu_0$.
As with $\tau_0$,  $\mu_0$ is assumed to fixed for a given gene, but may vary between genes.

Incorporating these explicit functional forms leads to representation of capped subsystem as: %\subsubsection{The capped subsystem with Assumed Forms for $\kappa(i)$, $\tau(i)$, and $\mu(i)$}\label{sec:capped_simp}
\begin{align*}
...
\end{align*}
and the decapped subsystem as,
%\subsubsection{The decapped subsystem with Assumed Forms for $\kappa(i)$, $\tau(i)$, and $\mu(i)$}\label{sec:decapped_simp}
\begin{align*}
...
With a closed-form expression for the determinant of the capped class matrix $\bs{U}$ we are now able to formulate another method to determine equilibrium solutions in the capped class by utilizing the matrix-vector formulation of the capped subsystem.
At equilibrium $\vec{m}'=0$, and so the matrix-vector equation (Eq. \ref{eq:matrix_capped}), when solved for the vector of state variables, $\vec{m}$, becomes,
\begin{equation}
\vec{m}=-\bs{U}^{-1}\vec{b}.
\end{equation}
...
Given that $\mu_0$ controls the rate at which capped mRNA become decapped for deletion, the physical interpretation of $\mu_0=0$ is no degradation of capped mRNA and thus no loss in the system, leading to linear, unbounded growth in capped mRNA populations equal to the mRNA transcription rate $\lambda$.
...
\begin{enumerate}%Exploration of the model it's parameters and their biological implication
	\item The capped system can be split into two components: Total transcript count and the distribution of transcripts. 
	\begin{enumerate}	
		\item The capped class transcript number is determined by $\lambda$, $\mu$
		\item In the capped class the total transcript abundance is determined by a similar relationship as Equation 21. %Except that the summation ranges from class 0 to imax instead of from 1 to imax. I am not sure that Equation 21 is correct.
		...
		\item First $\mu$ controls the rate of outflow from capped unto decapped, and second it shifts mRNAs to lower ribosomal classes.
		...
		\begin{enumerate}
			\item First it gives a determinant free solution to our system. Now, to obatin a full solution of the capped solution to our model we only need the first column of the Adjugate matrix.
	...
	\item  Ribosomal distribution on the decapped class is determined by the ratio of $\kappa$/$\tau$ and $\mu$
	\begin{enumerate}
	...
	\item The decapped class depends on the distribution of the capped class, $\tau$, $\mu$ and $\delta$
	\begin{enumerate}
		\item The decapped subsystem can be further divided into the $m_0^*$ class and the $m_i^*$ classes for $i>0$.
		...
		\item  $\delta$ only determines the accumulation of transcripts in the  $m_0^*$  class. It is reasonable to explore the model with   large $\delta$ since decapped transcripts are translationally incompetent. % $\delta\ might be more complex with a slightly different model design. In the current model formulation it mimics the primary (but not only) degradation with the cell, the 5' decapping and subsequent VCS /XRN1 5'-3' cotranslation degradation system. This ignores 3 other systems the two 3'- 5' pathways (CCR4-NOT3) and exosome, and the final slew of endodegradation pathways (NGD, NSD, NMD, ribothrypsis, and silencing).
