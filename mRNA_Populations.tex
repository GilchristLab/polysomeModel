...
Mature, but unbound and unmarked, mRNA are introduced to the system into mRNA class $0$ at a constant rate of $\lambda$.
...
If an unmarked mRNA has a ribosome load of $i$, it is defined as being in class $m_i$ and contributes to that class' abundance.
...
mRNAs in the unmarked classes can then transition freely between the $m_0$ and $m_{\imax}$ according to ribosome initiation and completion rates.

To begin the process of mRNA degradation, an mRNA must be tagged or marked.
A marked mRNA with a ribosome load of $i$ is denoted as being in class $m_i^*$.
Marking of an mRNA in class $i$ takes place at rate of $\mu(i)$.
Biologically $\mu$  represents different processes according to the class upon which it is acting on. 
Specifically, $\mu(0)$ represents deadenylation and decapping directly while $\mu(i)$ for $i > 0$ can represent a multitude of processes such as cotranslational decay, NMD, NGD, NSD or miRNA interference.
\mymarginpar{Ricardo, are you okay with this description of mRNA marking? If not, please correct.
  Also, would using 'tagged' be better than 'marked'?
  }
  Once an mRNA has been marked, it is assumed that no more initiation can take place and only translation completion occurs; this completion occurs until the mRNA's ribosome load drops to 0 and, thus, enters the $m_0^*$ class.
mRNA transcripts leave the system class $m_0^*$ at a constant rate of $\delta$.
...
\begin{figure} [htbp] \centering
  \begin{framed}
    %% Resize tikzpicture
    %% Code based on: https://tex.stackexchange.com/a/4340
    \resizebox{\textwidth}{!}
    {
      \begin{tikzpicture}[every node/.style={rectangle,fill=white,node distance=16ex, scale=0.75}]
		% Row 1
...
%\hat{m}_{\imax}=\dfrac{\prod_{i=1}^{\imax}\left(\kappa_0 \frac{i}{\GCD[\imax,i]}\right)}{\prod_{i=1}^{\imax}\left(\frac{\imax}{\GCD[\imax,i]}(\mu_0+\tau_0\frac{i}{\imax})+\kappa_0 \frac{i}{\GCD[\imax,i]}\right)},
%\end{equation}
...
%\hat{m}_{\imax}=\prod_{i=1}^{\imax}{\left(\frac{\imax}{i\kappa_0}\left(\mu_0+\tau_0\frac{i}{\imax}\right)+1\right)},
%\end{equation} 
\begin{align}
  \hat{m}_{\imax} &=\prod_{i=1}^{\imax}\frac{i \frac{\kappa_0}{\imax} }{\mu_0+\frac{i}{\imax}\left(\tau_0 + \kappa_0\right)},\label{eq:anyl_unmarked_imax_simp}\\
  &= \frac{\left(\frac{\kappa_0}{\imax}\right)^\imax \imax!}{\prod_{i=1}^{\imax} \mu_0+\frac{i}{\imax}\left(\tau_0 + \kappa_0\right)}
\end{align}
...
\begin{align}
  \det[\bs{U}] & =(-1)^{\imax+1}\prod_{i=0}^{\imax}{\left(\frac{i}{\imax}(\kappa_0)+(\mu_0+\tau_0\frac{i}{\imax})\right)}.\label{eq:U_det}
  %& =(-1)^{\imax+1}\mu_0\prod_{i=1}^{\imax}{\left(\frac{i}(\kappa_0/imax + \tau_0\frac{1}{\imax})+ \mu_0\right)}.\label{eq:U_det}
\end{align}
...
\begin{align}
  ...
  \det[\bs{F}] %&=(-1)^{\imax+1}\mu_0 \delta \lambda \tau_0\frac{1}{\imax}^{\imax}\left(\prod_{l=1}^{\imax}{-l}\right)\left(\prod_{i=1}^{\imax}{\mu_0 +\tau_0\frac{i}{\imax} + \frac{i}{\imax}\kappa_0}\right)
...
\begin{enumerate}%Exploration of the model it's parameters and their biological implication
	\item The unmarked system can be split into two components: Total transcript count and the distribution of transcripts. 
	\begin{enumerate}	
		\item The unmarked class transcript number is determined by $\lambda$, $\mu$
		\item In the unmarked class the total transcript abundance is determined by a similar relationship as Equation 21. %Except that the summation ranges from class 0 to imax instead of from 1 to imax. I am not sure that Equation 21 is correct.
		\begin{equation}\label{eq:eq_unmarked_sum}
			\frac{\lambda}{\mu}=\sum_{i=0}^{\imax}m_{i},
		\end{equation}
		\item Where $\lambda$ is only a scaling factor for the system as a whole. I.e. the distribution of transcripts across all classes is determined by $\kappa$, $\tau$, $\mu$ and $\delta$. %This matches my biological intuition. Translational regulation of gene expression only affects the proten production of a subset of expressed transcripts under a particular condition. This means that there are translationally regulated  and unregulated subsets. Regulated subsets change the distribution of ribosmes on the population of transcripts, while translationally unregulated subset's protein levels change with transcript abundance. Identifying members of these two subsets in different conditions is of great interest.
		...
		%mu has two effects. First it is one of the main determining factors of the overall abundance of transcripts. Second it weighs every category in the unmarked class except m imax. At high mu it quickly dominates moving transcripts quickly into the marked class allowing for low ribosomal loading. At small mu values it allows for translation. 1/mu is the primary degradation rate of the system and mostly specifies the half life of a transcript. 	
		\item The solution to the system, as presented previously, can be expressed in the determinant-adjoint form:
		\begin{equation*}
			\vec{m}=-\frac{1}{\det[\bs{U}]}Adj[\bs{U}]\vec{b}.
		\end{equation*}
		\item As $\vec{b}$ is [$\lambda$ 0 0 0 ... 0]. Only the first column of the adjoint matrix contributes to the result. 
		\begin{equation*}
		...
		\end{equation*}
		\item With this we can factor our solution into two parts: 1) the total transcript abundance and 2) The distribution of transcript across the ribosomal classes.
		\begin{equation*}
			\vec{m}=-\frac{\lambda a_{tot}}{\det[\bs{U}]} \frac{\vec{a}}{a_{tot}} = \frac {\lambda} {\mu} \vec{p}_m
		\end{equation*}
		\item The terms on the left hand side of the equation represent the total transcript population. The right hand side is the vector of probabilities, one entry for each class and is a function of $\kappa$, $\tau$, and $\mu$.
		\item This formulation has three interesting properties
		\begin{enumerate}
			\item First it gives a determinant free solution to our system. Now, to obatin a full solution of the unmarked solution to our model we only need the first column of the Adjugate matrix.
			\item Second it splits the two funtions of $\mu$; Its effect on transcript number and its effect on transcript distribution. And allows for their separate analysis.
			\item Third, it permits analysis of the underlying transcript distribution even under conditions where the model has no solution. For example, when $\mu$ = 0, both solutions are indeterminate. However, the determinant free solution allows for us to explore what the transcript distribution would be when $\mu$=0.
		\end{enumerate} 
	\end{enumerate}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\begin{equation}\label{eq:eq_unmarked_sum}
				\mu = \frac{ln(2)}{t_{1/2}}
			\end{equation}
			Where $t_{1/2}$ is the half-life. Model solutions were produced for a range of half-lives: 1, 5, 25, 50, 75, 95 and 99 percentile values of the half-lives found in Sorenson (2018)%These were12min, 23min, 59min, 109min, 220min, 830min, 4018min
			\item The resulting range of $\mu$ is from $10^-3 to 10^-6$. 
			% I would like to discuss the solution for the Adjugate in some part where it shows that there is a base solution for a system with only kappa and tau. Then when mu is added, this weights solutions towards smaller ribosomal classes. I am unsure where to put this. I will work on this after I get these initial results noted down in this document.
	...
	\begin{enumerate}
		...
		\item the $m_0^*$  class in the current model design represents only two forms of degradation, free mRNA degradation and the degradation of the remanents of XRN1 mediated degradation.% It could be futher extended to include more degradations mechanisms see note below
		\item  $\delta$ only determines the accumulation of transcripts in the  $m_0^*$  class. It is reasonable to explore the model with   large $\delta$ since marked transcripts are translationally incompetent. % $\delta\ might be more complex with a slightly different model design. In the current model formulation it mimics the primary (but not only) degradation with the cell, the 5' decapping and subsequent VCS /XRN1 5'-3' cotranslation degradation system. This ignores 3 other systems the two 3'- 5' pathways (CCR4-NOT3) and exosome, and the final slew of endodegradation pathways (NGD, NSD, NMD, ribothrypsis, and silencing).
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{Images/Figure1_biomodel_V2.png}
\caption{}
\end{figure}
\begin{enumerate}
...
Figure1
	\item The model captures some of the basic processes governing mRNA populations.
	\item Transcript production, degradation and the process of translation.
	\item Transcripts can exist in one of two states: capped and uncapped which captures the the role of the 5' cap in mRNA protection and translation initiation. 
	\item Capped transcripts are translationally competent, meaning that new ribosome can be loaded onto the transcript. 
	\item Individual transcripts in the cell will be found with a set number of ribosomes (none, 1, 2, etc).
	\item The number of ribosomes on a transcript determines that transcripts ribosomal class.
	\item The model seeks to determine how the population of transcipts of a single gene are distributed between ribosomal classes and capped and uncapped states.
	\item Transcripts enter into the model as defined by the transcription rate $\lambda$ into the capped state with no ribosomes (class 0)
	\item From capped class 0 a transcript can have two fates. 
	\item The transcript can be marked for degradation at rate $\mu$ and move into the uncapped class 0.
	\item Alternatively, a ribosome can initiate translation at rate $\kappa*(1-i/imax)$ and be loaded onto the transcript and move it into capped class 1.Where i is the current transcript class and imax is the maximal ribosomal occupancy on the transcript
	\item A ribosome on transcript can then elongate and terminate at a rate of $\tau * i$. After a ribosome fully elongates and terminates it leaves the transcript and the transcript falls to a lower class.
	\item Capped transcripts move through rounds of translation initiation and elongation\/termination and distribute along the different ribosomal classes. 
	\item From any ribosomal class in the capped state the transcript can be marked at rate $\mu$  and move into the uncapped state while maintaining the same ribosomal class.
	\item Uncapped transcripts can no longer initate new rounds of translation, but allow for currently loaded ribosomes to complete translation. 
	\item This process represent co-translational decay, a common method of mRNA decay in eukaryots (Citation) 
	\item After all ribosome complete translation, the mRNA is in uncapped class 0 and completely degraded at a rate $\delta$.

Supplemental Figure 1
	\item The model solution at equilibrium has three outputs.
	\item First the total number of mRNA transcripts.  Second split between the capped and uncapped states. And third the distribution of the transcripts across ribosomal classes for each of the states.
	\item 
...
\begin{enumerate}
	...
	\item Alternatively a probabilistic model of ribsomal position given a particular load could allow for redistribution of capped transcripts into uncapped states of lower ribosomal class.
...
\end{enumerate}
	...
	\item Model predicts mRNA distribution across ribosomal classes
	\begin{enumerate}
		\item The model 
		\item Density independent initiation and density independent initiation models
		\item The model predicts the steady state total mRNA quantity in both marked and unmarked states as well as the mRNA's distribution across different ribosomal classes in each state.
		\item  Figure X Left, 
	\end{enumerate}
	\item The model provides an upper bound on translational efficiency
	\begin{enumerate}
		...
		\item The degradation rate ($\delta$) only controls the transcript population in the marked zeroth class. To reduce interference from this we set a very high degradation rate.
...
To establish the base model behavior, DII and DDI models were run with a long half-life across a broad range of scaled initiation on a transcript encoding a protein of median length (369aa) (Figure 1). In the DII model, all ribosomal classes are populated. At the high end of the biological scaled initiation transcripts reach full ribosomal occupancy (Figure 1A). The DII model ignores initiation interference caused by ribosomes already present on the transcript. The DDI model accounts for initiation interference, and as expected, a decrease in ribosomal load is observed compared to DII. Even with interference, access to most ribosomal classes is attainable in the DDI model. Ribosomal profiling experiments suggest that most transcripts have one to ten ribosomes loaded with a strong preference for smaller polysomes. Live single molecule fluorescence experiments do show higher polysome numbers rising into the 50s [33]. For the long reporters used in these experiments that equates to a 12 to 30 percent ribosomal load. Thus the DDI model probably overestimates the loading for a particular scaled initiation rate.
	\end{enumerate}
	\item Marking rates have only a mild effect on ribosomal load, but a large effect on protein production.
	\begin{enumerate}
		\item From the solution of the unmarked system we can define two separate effects of transcript marking.
		\begin{enumerate}
			\item  First, marking determines how many transcripts can accummulate in the unmarked class, thus determining the total transcript population		
			\item Second, marking rate shifts the distribution of transcripts  in the unmarked class towards lower ribosomal classes.	
		\end{enumerate}
		\item Marking only has a mild effect on the distribution of transcripts on the unmarked class


	\end{enumerate}
	\item Under equilibrium protein length affects ribosomal load but not ribosomal density
	\begin{enumerate}
		\item stuff
	\end{enumerate}
...
%Result 3: Marked class has only a limited effect of the total distribution of mRNAs and overall protein production.
...
%add new marked class results here. Marked class in its current formulation is very unpopulated. 
...
%	Marked 
%		M0* is delta driven
%		All other M* have a set behaivior dependent on the distribution of transcripts in unmarked.
