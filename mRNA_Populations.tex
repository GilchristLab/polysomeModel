...
While the individual components of $\vec{p}$ are functions of $i$, \imax, the translation initiation rate $\kappa$, the elongation rate $\tau_0$ and $\mu$ and have no closed form solution, it is worth noting that because the probability distribution of the capped population must sum to 1.
...
\begin{equation}\label{eq:Expected_ribo_load}
  E_\mvecstar\left(\ribosome\right) =\sum_{i=0}^{\imax}i \times p^*_{i}
  E_\mvec\left(\ribosome\right) =\sum_{i=0}^{\imax}i \times p_{i}.
\end{equation}
