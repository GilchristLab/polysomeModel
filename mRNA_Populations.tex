...
\begin{align}
\hat{m}_{\imax} &=\prod_{i=1}^{\imax}{\left(\frac{\imax}{i\kappa_0}\left(\mu_0+i\tau_0\right)+1\right)},\label{eq:anyl_unmarked_imax_simp}
\end{align}
for any level of \imax. \mymarginpar{mikeg: 10/22/19 Note we no longer have any GCD since they cancelled out.  See commented out text BEGIN CRITICAL NOTE BY MIKE below
...
\begin{align}
  Det[\bs{U}] & =(-1)^{\imax+1}\mu_0\prod_{i=1}^{\imax}{\left(\frac{i}{\imax}(\kappa_0)+(\mu_0+i\tau_0)\right)}.\label{eq:U_det}
...
\begin{align}
  ...
  Det[\bs{F}] %&=(-1)^{\imax+1}\mu_0 \delta \lambda \tau_0^{\imax}\left(\prod_{l=1}^{\imax}{-l}\right)\left(\prod_{i=1}^{\imax}{\mu_0 +i \tau_0 + \frac{i}{\imax}\kappa_0}\right)
            ...
            &=-\mu_0 \delta \lambda \tau_0^{\imax}\imax!\right)\left(\prod_{i=1}^{\imax}{\mu_0 +i \left(\tau_0 + \frac{\kappa_0}{\imax}\right)}\right)..
\end{align}
