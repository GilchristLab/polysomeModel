...
\begin{equation}
  \label{eq:\kappa_i}
  \kappa_i = \kappa_0 \left(1- \frac{i}/\imax),
\end{equation}
...
\clearpage
  ...
  \begin{enumerate}
    \item  The analytical steady state solution ~\ref{eq:decapped_solution} can be thought of in two parts. 
    \item $m_0^*$ and the remaining $m_i^*$ for $i>0$.
    \item The whole system is again scaled by the transcription rate $\lambda$.
    \item $m_0^*$ is solely determined by the mRNA clearance rate $\delta$.
    ...
    \item  Figure 4A shows the mRNA distribution in the decapped state for four different scaled initiation rates $\kappa'$, for a median length protein with a low marking rate ($2\times10^{-4}$) and the full range of $kappa'$ are shown in the heatmap in Figure 4B.
...
\clearpage
  ...
  \item Steady states solution for the full system
  \begin{enumerate}
    \item The full system combines the mRNA distributions from the capped and decapped states.
    \begin{enumerate}
      \item Figure 5A shows the mRNA distribution in the full system for three values of $\kappa'$, for a median length protein with a low marking rate ($2\times10^{-4}$) and the full range of $\kappa'$ are shown in the heatmap in Figure 5B.
      \item The system is unimodal at low $\kappa'$ when the capped and decapped distributions overlap (Figure 5A mid and low). 
      \item The full distribution becomes bimodal as $\kappa'$ increases (Figure 5A high).
    \end{enumerate}
  \end{enumerate}
...
\caption{mRNA distribution in the full system. A) Distribution profiles for three scaled initiation values low) $1\times 10^{-3}$ mid) $2\times 10^{-2}$ and high) $1\times 10^{-1}$ B) Heatmap of model output across a range of scaled initiation values. Lines represent slice represented in A). Results produced with \imax of 39 and a low marking rate of $2\times10^{-4}$  (99\textsuperscript{th} percentile). Color bar shows probability of finding mRNA in particular ribosomal class.}
%\centering Red: decapped class, Green: capped class, Blue: Total= capped+decapped classes}
\end{figure}
\clearpage
...
\begin{enumerate}
  \item To explore the role of mRNA stability on mRNA populations we varied the marking rate $\mu$ from the 1\textsuperscript{st}, 50\textsuperscript{th} and 99\textsuperscript{th} percentile values as determined from  Presnyak 2015.
...
\caption{Higher marking rates $\mu$ reduce ribosome load in the capped system in yeast.  A)  Heatmaps for the full system. Left) low $\mu$ ($2\times 10^{-4}$ 1/s) Center) median $\mu$ ($1.7\times 10^{-3}$ 1/s) Right) $\mu$ ($5.7\times 10^{-3}$ 1/s)  B) individual density profiles for low (0.001), mid (0.01) and high (0.1) scaled initiation values for each $\mu$ All results calculate with \imax = 39.}
%\centering Red: decapped class, Green: capped class, Blue: Total= capped+decapped classes}
\end{figure}
\clearpage
...
\begin{enumerate}
  \item This is highlighted by the current gold standard study of mRNA half-lives in the model organism \textit{Arabidopsis thaliana}, where the marking rate $\mu$ measured are ten to one hundred times lower than those in yeast. 
  \item To explore the effect of lower $\mu$ in Arabidopsis, we ran the model using the same scaled initiation rates as in yeast, the median Arabidopsis \imax of 41. 
...
\caption{Low marking rates $\mu$ in Arabidopsis result in a smaller effect on mRNA distributions in the capped system.  A)  Heatmaps for the full system. Left) low $\mu$ ($7.7\times 10^{-6}$ 1/s) Center) median $\mu$ (1$\times 10^{-4}$ 1/s) Right) $\mu$ ($1\times 10^{-3}$ 1/s) B) individual density profiles for low (0.001), mid (0.01) and high (0.1) scaled initiation values for each $\mu$. All results calculate with \imax = 41.}
%\centering Red: decapped class, Green: capped class, Blue: Total= capped+decapped classes}
\end{figure}
\clearpage
...
\begin{enumerate}
\item Gene specific MRL ~\ref{eq:MRL} were calculated for the genes analyzed in Duc and Song 2018 and compared to the empirical MRL calculated from raw data from Weinberg 2016.
\begin{enumerate}
  \item Model predictions of MRL showed a significant positive correlation to empirical MRLs.
  \item This result is reassuring as the model performs well despite no model fitting being performed.
  \item 
  ...
  \item All of which align with low to mid scaled initiation rate MRL predictions. 
  \item Finally, mRNA distributions for the full system agree with signal from polysome traces (Lokdarshi 2020, Dasgupta 2023).
\end{enumerate}
...
\subsection{At steady state protein production per transcript is independent of protein length}
\begin{enumerate}
\item At steady state the MRL increases with protein length, for the same values of marking rate $\mu$ and scaled initiation rate $\kappa'$ (Figure 9A).
\begin{enumerate}
  \item This is misleading with regards to per transcript protein production. 
  \item By the definition of steady state the same number of ribosomes must initiate and terminate translation on a transcript. 
  \item The higher MRL for longer proteins is due to the fact that more ribosomes can be loaded on a transcript at one time.
  \item However, that does not affect the rate ribosomes initiate and terminate. 
  \item Mean ribosomal density, i.e. the MRL/\imax, is a better proxy for understanding per transcript protein production between transcripts of different lengths (Figure 9B). 
  \item The mean ribosomal density represent the fraction of the transcript actively producing protein, and thus is invariant to length.
  \begin{enumerate}
    \item
...
\caption{The mean ribosomal density on a transcript is independent of their length. A) MRL per transcript is higher for longer transcripts. B) When the MRL is corrected for the length of the transcript, the  mean ribsomal density collapses to the same curve for all transcripts under the same parameters. For this plot Arabidopsis parameters were used \imax =  8 (1\textsuperscript{st} percentile),  40 (50\textsuperscript{th} percentile), 164 (99\textsuperscript{th} percentile), low marking rate ($7.7\times 10^{-6}$ 1/s), over the full scaled initiation range 0.0001- 0.5.}
%\centering Red: decapped class, Green: capped class, Blue: Total= capped+decapped classes}
\end{figure}
\clearpage
...
\begin{enumerate}
\item As shown in previous results, higher marking rates $\mu$ lead to lower MRL in the capped state and increase mRNA abundance in the decapped state.
\begin{enumerate}
  \item Using ~\ref{eq:log_odds}, we can determine how much of the mRNA population is in the capped state.
  \item We produced output across all scaled initiation values $\kappa'$ and under the 1\textsuperscript{st}, 50\textsuperscript{th} and 99\textsuperscript{th} percentiles for marking rates in both yeast and Arabidopsis (Figure 10). 
  ...
  \item Secondly and similarly, higher $\mu$ biases transcripts towards the decapped class as previously seen in Figures 6 and 7.  
\end{enumerate}
...
\caption{Percentage of mRNA in the capped state for a range of marking rates in yeast ( low $\mu$ ($2\times 10^{-4}$ 1/s), median $\mu$ ($1.7\times 10^{-3}$ 1/s), high $\mu$ ($5.7\times 10^{-3}$ 1/s)) and Arabidopsis( low $\mu$ ($7.7\times 10^{-6}$ 1/s, median $\mu$ (1$\times 10^{-4}$ 1/s), high $\mu$ ($1\times 10^{-3}$ 1/s)). }
%\centering Red: decapped class, Green: capped class, Blue: Total= capped+decapped classes}
\end{figure}
\clearpage
...
\begin{enumerate}
\item Protein production is a function of the rate at which ribosomes leave the transcript and the underlying total number of transcripts.
\begin{enumerate}
  \item When the scaled initiation rate $\kappa'$ is low, the average protein production rate (PPR) for the capped state could be estimated by multiplying the total number of transcript by the initiation rate. At higher $\kappa'$ this would be an upper limit.
  \item However, this ignores the DDI and marking effects in the system and the actual PPR will be lower. 
  \item Contributions from the decapped state are harder to estimate from the parameters as the distribution is dependent on the capped state's distribution.
\end{enumerate}
\item Another way of comparing protein production is to calculate the relative protein output (RPO) \ref{eq:Expected_ribo_load}, MRL $\times \sum{\mvechat}$ . 
\begin{enumerate}
  \item This uses the MRL as a proxy for protein production rate. To compare across transcripts of different length you can divide by \imax.
  \item The full systems protein production can be found with \ref{eq:System_ribo_load}.
  \item The RPO of the capped and decapped state is dependent on both the MRL and the total transcript population of each state.
  \item A property of our model is the the decapped state MRL $\leq$ the MRL of the capped state (Figure 11A).
  \item As $\kappa'$ increases the RPO will increase as well (Figure 11B).
  \item The marking rate $\mu$ plays an important role in determining the capped MRL and transcript abundance in the two states (Figure 11B).
  \item  Each of the three cases in Figure 10 B, has been broken down into the RPO contributions from the capped and decapped states (Figure 11 C-E). 
  \item A surprising finding from out model is that when $\mu$ is low ($2\times 10^{-4}$ 1/s), almost half of all protein production can arise from the decapped state.
  \item This can be seen inf Figure 11 F, where 41\% of all protein arises from the decapped state.
...
\caption{Estimated average protein production in yeast. A) Ratio of the MRL of the decapped and capped states for a protein of \imax 40 and low marking rate ($2\times 10^{-4}$ 1/s) at values of capped MRL of 0-40. B) Protein production across different marking rates  low $\mu$ ($2\times 10^{-4}$ 1/s), median $\mu$ ($1.7\times 10^{-3}$ 1/s), high $\mu$ ($5.7\times 10^{-3}$ 1/s). Total protein production is normalized to the maximal protein production across all parameters. C-F) Contribution of capped and decapped states to total protein production. C) Low marking D) Medium marking E) High marking F) zoom in of high marking result. }
%\centering Red: decapped class, Green: capped class, Blue: Total= capped+decapped classes}
\end{figure}
\clearpage
