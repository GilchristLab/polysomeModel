...
\item This is a common pathway in many organisms and accounts for decay in $\sim 68\%$ of Arabadopsis genes (Sorenson 2018).
  \mmpar{I uncommented this point, can you reconcile it with the previous sentence?}    
...
\begin{enumerate}
\item  In addition to being more biologically realistic, structuring the mRNA population by its polysome classes (ribosome load) and the status of its 5' cap allows us to understand how the the rates mRNA production $\lambda$, decapping $\mu$, protein elongation $\tau$, and the clearance rate $\delta$ decapped and ribosome free mRNAs $\mhatstar_0$ shape the steady state distribution of a gene's mRNA population across polysome classes and capping state.
  Overall, we find that 
  \begin{enumerate}
  \item Capped mRNA population
    \begin{enumerate}
    \item Analytical and numerical solutions show transcription rate $\lambda$ acts as a scaling factor such that the abundance \mmpar{abundance or density?} of all of the mRNA polysome classes are proportional to $\lambda$.
      For example,
    \item The sum of the of capped mRNA classes $\mvechat$ is solely determined the ratio of $\lambda$ to the 5' decapping rate $\mu$, i.e. $\sum_{i = 0}^{i_{\imax}} \mhat_i= \lambda/ \mu$.
      \begin{enumerate}
      \item This result suggests that, under biologically realistic conditions, as $\mu$  decreases, the capped population increases. Further exploration of the empirical data used for this model suggest that $\mu$ buffers $\lambda$, with most genes implying a constant $\lambda$  (supplemental figure 1).
      \end{enumerate}
    ...
    \item Impact of $\kappa/\mu$ \mmpar{Turn into a topic sentence.}
      \begin{enumerate}
...
\begin{figure}[!ht]
\centering
\includegraphics[width = 135mm]{Images/lambdavsmu_log.png}
\caption{Marking rate determines the gene abundance for most transcripts. The parameter $\lambda$ was calculated by multiplying the RNA-seq RPKM ($\sum_{0}^{\imax} \mvechat_i$) values of Weinberg 2016 with the marking rates $\mu$ obtained from Presnyak 2015 as is described in ~\ref{eq:capped_solution}. As $\mu$ increses, $\lambda$ increases linearly for many genes, suggesting $\mu$ accounts for the accumulation of transcripts. Some genes significanly deviate from this pattern, indicating additional production from $\lambda$. Spearmans $\rho$ is presented with a pvalue  $  < 10^{-16}$.}
%\centering Red: decapped class, Green: capped class, Blue: Total= capped+decapped classes}
\end{figure}
\clearpage
